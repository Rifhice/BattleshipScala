...
class Player(val name: String, val isHuman: Boolean, val input: (Random) => Position, val playerState: PlayerState){
    def shoot(position: Position, player: Player): (Player, Player, Int) = {
        ...
        if(hittedBoat.length != 0){
            ...
            if(hittedBoat(0).isDead) (newPlayer, newOpponent, 2) else (newPlayer, newOpponent, 1)
        }
        else{
            (new Player(name, isHuman, input, PlayerState(playerState.boats, playerState.hitPosition, playerState.shotPosition :+ position, playerState.hitShotPosition)), newOpponent, 0)
        }
    }
